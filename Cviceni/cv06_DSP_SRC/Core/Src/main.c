...
//		DataTransmit2MTLB(40001,(uint8_t *) &teplota[nTeplota], 1);
	...
	case 35000:
		...
		uint32_t x_time = FindTimeStamp('Y') - FindTimeStamp('X');

		DataTransmit2MTLB(35000, (uint8_t *)x3, 1000);

		while(s2m_Status)// toto neni dobre reseni...
					{
					}
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
  if(GPIO_Pin==B1_Pin)
  {
	  ...
	  else
	  {
		  ...
		  DataTransmit2MTLB(10001, (uint8_t *) &butt_transfer_time, 1);
	  }
...
/**
// Počet koeficientů filtru
#define NUM_COEF 11

// Koeficienty filtru vygenerované v MATLABU
const float fir_coef[NUM_COEF] = {
		-0.023807064296002131836837989453670161311f,
		 0.031135834400189601328667876600775343832f,
		 0.082627381107522157588363143077003769577f,
		 0.148407485940645150757077885828039143234f,
		 0.203350384636836856167008136253571137786f,
		 0.224744081340164686411142724864475894719f,
		 0.203350384636836856167008136253571137786f,
		 0.148407485940645150757077885828039143234f,
		 0.082627381107522157588363143077003769577f,
		 0.031135834400189601328667876600775343832f,
...
static float history[NUM_COEF] = {0.0f};

//static uint32_t n;
**/

// --- Parametry filtru ---
#define NUM_STAGES 5    // Počet "biquad" sekcí (např. 1 pro filtr 2. řádu)
#define BLOCK_SIZE 64
// Instance filtru
arm_biquad_cascade_df2T_instance_f32 S_R;

static float32_t pCoeffs[NUM_STAGES * 5] = {
		    // --- Sekce 1 ---
		    // G1 = 0.04697057667...
		    // b0,b1,b2 = 1, 2, 1
		    // a1,a2 = -1.684..., 0.872...
		    0.04697057667412158f,  // b0*G1
		    0.09394115334824316f,  // b1*G1
		    0.04697057667412158f,  // b2*G1
		    1.6842446644939943f,   // -a1
		    -0.8721269711904805f,  // -a2

		    // --- Sekce 2 ---
		    // G2 = 0.04187764835...
		    // b0,b1,b2 = 1, 2, 1
		    // a1,a2 = -1.501..., 0.669...
		    0.04187764835816771f,  // b0*G2
		    0.08375529671633541f,  // b1*G2
		    0.04187764835816771f,  // b2*G2
		    1.5016252897669733f,   // -a1
		    -0.6691358831996440f,  // -a2

		    // --- Sekce 3 ---
		    // G3 = 0.03834126433...
		    // b0,b1,b2 = 1, 2, 1
		    // a1,a2 = -1.374..., 0.528...
		    0.03834126433168550f,  // b0*G3
		    0.07668252866337100f,  // b1*G3
		    0.03834126433168550f,  // b2*G3
		    1.3748196094891327f,   // -a1
		    -0.5281846668158747f,  // -a2

		    // --- Sekce 4 ---
		    // G4 = 0.03612488232...
		    // b0,b1,b2 = 1, 2, 1
		    // a1,a2 = -1.295..., 0.439...
		    0.03612488232307171f,  // b0*G4
		    0.07224976464614342f,  // b1*G4
		    0.03612488232307171f,  // b2*G4
		    1.2953458231997490f,   // -a1
		    -0.4398453524920359f,  // -a2

		    // --- Sekce 5 ---
		    // G5 = 0.03505939977...
		    // b0,b1,b2 = 1, 2, 1
		    // a1,a2 = -1.257..., 0.397...
		    0.03505939977279917f,  // b0*G5
		    0.07011879954559834f,  // b1*G5
		    0.03505939977279917f,  // b2*G5
		    1.2571403459100337f,   // -a1
		    -0.3973779450012304f   // -a2
		};
static float32_t pState[NUM_STAGES * 4];
/*
 * init pomoci
 * Parameters
...
void Init_IIR()
{
    // Inicializace filtru pro pravý kanál
    arm_biquad_cascade_df2T_init_f32(
        &S_R,       // Ukazatel na instanci
        NUM_STAGES, // Počet sekcí
        pCoeffs,    // Ukazatel na koeficienty
        pState      // Ukazatel na state buffer
    );

    // v budoucnu i pro levý kanál?
    //...
}
...
void ProcessSamples()
{
	arm_copy_f32(sample_L, sample_L1, BLOCK_SIZE);

	arm_biquad_cascade_df2T_f32(&S_R,sample_L,sample_R1,BLOCK_SIZE);
