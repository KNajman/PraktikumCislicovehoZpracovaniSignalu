...
{
	if(nn>0)		disp_Data_buff[nn]=disp_Data_buff[nn-1];
	else			disp_Data_buff[nn]=disp_Data_buff[255];
}
void predchoziCLK(uint8_t nn)
{
	if(nn>0)		disp_CLK_buff[nn]=disp_CLK_buff[nn-1];
	else			disp_CLK_buff[nn]=disp_CLK_buff[255];
}

void CLKlow() {	disp_CLK_buff[disp_n]=0;		predchoziData(disp_n);  disp_n++;}
void CLKhigh(){	disp_CLK_buff[disp_n]=1;		predchoziData(disp_n);  disp_n++;}
void SDOlow() { predchoziCLK(disp_n); disp_Data_buff[disp_n]=0;      disp_n++;}
void SDOhigh(){ predchoziCLK(disp_n); disp_Data_buff[disp_n]=1;      disp_n++;}
...
void disp_DataOut(uint8_t *tm1637_TxBuffer)							//Low level data transfer function
{
	for(int8_t j = 0; j < 8; j++)								//Send least significant bit first
	{
		CLKlow();
		predchoziCLK(disp_n);
		disp_Data_buff[disp_n]=tm1637_TxBuffer[j]; disp_n++;							//Check logic level
		CLKhigh();
	}
}
...
{
	if(disp_X) return;//zastavit, protoze bezi vysilani
	disp_n=1;
	if(Br > 8)Br = 7;												//there are 7 levels of brightness
	...
	{
		disp_CLK_buff[i]=1; 		disp_Data_buff[i]=1;
	}
	disp_X=1;
...
{
	uint8_t x[4];
	x[0]=char2segments(text4char[0]);x[1]=char2segments(text4char[1]);x[2]=char2segments(text4char[2]);x[3]=char2segments(text4char[3]);
...
{
	if(hour>23) hour=0; //minut-60;
	if(minut>59) minut=0; //minut-60;
	if(second>59)second=0;
	h1=hour/10; h0=hour-h1*10;
	m1=minut/10; m0=minut-m1*10;
	s1=second/10; s0=second - s1*10;
}
void TM1637_Time_GET(uint8_t *hour, uint8_t *minut, uint8_t *second)
{
	hour[0]=h1*10+h0; minut[0]=m1*10+m0; second[0]=s1*10+s0;
}
...
void TM1637_Show_Time_MMSS_Next(void)
{
	...
	x[2]=char2segments(s1+48);
	x[3]=char2segments(s0+48);
	if(!Show_Time_paused)TM1637_display_MyData(x, 8);
}
...
void TM1637_Show_Time_HHMM_Next(void)
{
	...
	if(s0 & 0x01) x[1]=char2segments(h0+48) + 0x80;//+dvojtecka
	else x[1]=char2segments(h0+48);
